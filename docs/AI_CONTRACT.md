...
AI summary records are separate user-scoped Firestore health data, not local usage history. Validated summaries are saved to the source document (`dailyNutritionSummaries/{date}`, `symptomLogs/{logId}`, or `weeklySummaries/{weekId}`) and to `users/{uid}/aiSummaries/{summaryId}` for future analysis. Daily and weekly documents also store AI nutrition estimates with `nutritionProcessedBy: "ai"` and `nutritionProcessingStatus: "ai_processed"` when the AI returned nutrition estimates. Repeat daily or weekly requests reuse those AI-processed records instead of sending another provider request for the same date or ISO week.

When a local daily nutrition summary is regenerated after an AI summary has already been saved, the local write must use a merge update so it does not delete `aiSummary`, `aiNutritionTotals`, `aiNutritionProcessed`, or `nutritionProcessingStatus`. Loaded daily summaries and weekly/report nutrition views should prefer `aiNutritionTotals` whenever `aiNutritionProcessed` is true; this prevents repeat daily or weekly AI calls from recalculating nutrition that was already handled and saved by AI.
